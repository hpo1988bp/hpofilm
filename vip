...
def GetSheetIDFromSettings():
	sid = "1l7W1idOLLxgLA7EinuwGimHa7kDEWWVKOUYo4MxPp8Q"
	...
	for row in _json["table"]["rows"]:
		...
		if "plugin://" in item["path"]:
			...
			if "spreadsheets/d/" in item["path"]:
				# https://docs.google.com/spreadsheets/d/1l7W1idOLLxgLA7EinuwGimHa7kDEWWVKOUYo4MxPp8Q/edit#gid=0
...
def get_playable_url(url):
	if "youtube" in url:
		...
		get_thvl = "https://docs.google.com/spreadsheets/d/1l7W1idOLLxgLA7EinuwGimHa7kDEWWVKOUYo4MxPp8Q/export?format=tsv&gid=1287121588"
		try:
			(resp, content) = http.request(
				get_thvl, "GET"
			)
		...
		get_sphim = "https://docs.google.com/spreadsheets/d/1l7W1idOLLxgLA7EinuwGimHa7kDEWWVKOUYo4MxPp8Q/export?format=tsv&gid=1082544232"
		try:
			(resp, content) = http.request(
				get_sphim, "GET"
			)
